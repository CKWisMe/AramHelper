...
const CHAMPION_TTL_MS = 10 * 60 * 1000;
...
async function getMeta(forceRefresh = false) {
  if (!forceRefresh && cache.meta && cache.meta.expiresAt > now()) {
  ...
  const championByKey = new Map(champions.map((champion) => [champion.key, champion]));
  ...
  cache.meta = {
    data: meta,
    expiresAt: now() + STATIC_TTL_MS
  };

  return meta;
}

function resolveChampion(meta, query) {
  const normalizedQuery = normalizeQuery(query);
  if (!normalizedQuery) {
    return null;
  }

  const exact = meta.champions.find((champion) => champion.normalized.includes(normalizedQuery));
  if (exact) {
    return exact;
  }

  return meta.champions.find((champion) => champion.normalized.some((alias) => alias.includes(normalizedQuery)));
}

function buildSuggestions(meta, query) {
  const normalizedQuery = normalizeQuery(query);
  if (!normalizedQuery) {
  ...
  return meta.champions
    .filter((champion) => champion.normalized.some((alias) => alias.includes(normalizedQuery)))
    .slice(0, 8)
    .map((champion) => ({
...
async function getChampionData(query) {
  const meta = await getMeta();
  const champion = resolveChampion(meta, query);
  ...
  if (!champion) {
    return {
      ...
      body: {
        ...
        message: `找不到「${query}」對應的英雄。`,
        suggestions: buildSuggestions(meta, query),
        patch: meta.patch
      }
    };
  }

  const cacheKey = `${meta.patch.statsPatch}:${champion.key}`;
  const cached = cache.champion.get(cacheKey);
  ...
  cache.champion.set(cacheKey, {
    data: body,
    expiresAt: now() + CHAMPION_TTL_MS
  });
  ...
  try {
    ...
    if (request.method === 'GET' && url.pathname === '/api/meta') {
      ...
      sendJson(response, 200, {
        patch: meta.patch,
        featured: meta.featured.map((champion) => ({
          nameZh: champion.nameZh,
          nameEn: champion.nameEn,
          slug: champion.slug,
          imageUrl: champion.imageUrl
        })),
        champions: meta.champions.map((champion) => ({
          nameZh: champion.nameZh,
          nameEn: champion.nameEn,
          slug: champion.slug
        }))
    ...
    if (request.method === 'GET' && url.pathname === '/api/champion') {
      ...
      const result = await getChampionData(query);
...
server.listen(PORT, () => {
  console.log(`ARAM live helper running at http://127.0.0.1:${PORT}`);
});
